...
///////////////////////////////////////////////////////////////////////////////////
// queue needed to sort number into new number array from the binary tree
typedef struct q_rep *queue;
...
static void join_queue (queue q, BSTree t);
static void leave_queue (queue q);
static void drop_queue (queue q);
...
{	
	int i = 0, x = BSTreeNumNodes(t);
	queue new_q = new_queue();
	join_queue(new_q, t);

	while (i < x && new_q->head != NULL) {
		if (new_q->head->left != NULL)
			join_queue(new_q, new_q->head->left);
		if (new_q->head->right != NULL)
			join_queue(new_q, new_q->head->right);
		printf("%d ", new_q->head->value);
		i++;
		leave_queue(new_q);
	}
	drop_queue(new_q);
...
static queue new_queue (void) 
{
	queue n_queue = malloc(sizeof *n_queue);
	if (n_queue == NULL) err (EX_OSERR, "since NULL, could not allocate new queue");
	n_queue->head = NULL;
	n_queue->tail = NULL;
	return n_queue;
}
...
{	
	...
	// setup new node
	q_node *n_node = malloc(sizeof *n_node);
	if (n_node == NULL) err (EX_OSERR, "since NULL, could not allocate new queue node");
	assert(t != NULL);
	n_node->value = t->value;
	n_node->next = NULL;  //*n_node = (q_node) { .value = num, .next = NULL };
	n_node->left = t->left;
	n_node->right = t->right;

	// add this node into queue, add new node to queue's tail
	if (q->head == NULL)
		// queue is empty. the new node would be the first item ("q->head")
		q->head = n_node;
	if (q->tail != NULL)
		// queue is not empty
		q->tail->next = n_node;
